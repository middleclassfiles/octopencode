...
If `node bin/hydra channel send ...` returns `Target terminal not found`, that means you skipped worker creation. Stop, run the spawn commands, and verify the workers exist before doing anything else.
...
```bash
node bin/hydra channel send <workerTerminalId> "STATUS?" --from {{terminalId}}
```
...
```bash
node bin/hydra channel list {{terminalId}}
```

Send a message to a worker:
```bash
node bin/hydra channel send <workerTerminalId> "your message" --from {{terminalId}}
```
