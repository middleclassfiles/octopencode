...
![Last Update](https://img.shields.io/github/last-commit/hesamsheikh/hydra?label=Last%20Update&style=flat-square)
...
This repo is a personal exploration of what an AI coding environment might look like when terminal coding agents are treated as parts of a bigger orchestration layer, not the final interface by themselves. The point is not to hide **opencode** behind abstractions. The point is to make *multi-agent work less chaotic for the developer* on a real codebase.
...
- **Runs multiple opencode terminals** so one developer can coordinate several coding sessions at once
...
A **tentacle** is a folder under `.hydra/tentacles/<tentacle-id>/` that holds agent-readable markdown such as `CONTEXT.md`, `todo.md`, and any extra notes needed for that slice of the codebase.
...
In Hydra, a tentacle is not only a task bucket. It is also where the job keeps its local context. That can include notes about one part of the codebase, implementation details, handoff files, and a `todo.md` that tracks what still needs to happen. An opencode agent can read and update those files as the work moves forward.
...
## Opencode Managing Opencode

One of the main ideas here is that **opencode** should not only be treated as a single terminal session waiting for a human prompt. In Hydra, one opencode agent can coordinate other opencode agents, assign them specific jobs, and exchange short messages with them while the human stays at the orchestration layer.

This is different from opencode's built-in subagent spawning, since it allows you to directly see and control what each worker agent is doing.

That means Hydra is not just a dashboard for multiple terminals. It is also a way to structure parent-worker behavior around scoped tasks and shared context files.
...
Deck reads the tentacle files directly, parses checkbox items from `todo.md`, and uses incomplete items to generate worker prompts. Hydra's opencode plugin events feed the API with agent state, transcript, and idle events so the UI can show more than raw terminal output.
...
```bash
Hydra is not published to the npm registry yet.
```
...
The registry install flow `npm install -g hydra` will only work after the package is published.

</details>

On first run, **Hydra** creates the local `.hydra/` scaffold automatically, assigns a stable project ID, picks an available local API port starting at `8787`, and opens the UI unless `HYDRA_NO_OPEN=1` is set.

## Requirements

- Node.js `22+`
- `opencode` installed and logged in for the supported agent workflow
- `git` for worktree terminals
- `gh` for GitHub pull request features
- `curl` for the opencode plugin event callbacks

Startup fails when the `opencode` binary is unavailable (see https://opencode.ai for installation).

## What persists

- `.hydra/` keeps project-local scaffold and worktrees
- `~/.hydra/projects/<project-id>/state/` keeps runtime state, transcripts, monitor cache, and metadata
- `.hydra/tentacles/<tentacle-id>/` keeps the context files and todos that agents read

PTY sessions survive browser reloads during the idle grace period, but they do **not** survive an API restart. Hydra marks previously running terminal records as `stale` on startup when it cannot reattach them to a live PTY session; use `hydra terminal list`, `stop`, `kill`, and `prune` to inspect and clean them up. Hydra caps live PTY sessions at 32 by default to protect the host; set `HYDRA_MAX_TERMINAL_SESSIONS` to a positive integer to tune that limit for larger orchestration runs.
